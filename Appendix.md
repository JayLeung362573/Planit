A collection of info not included in our M0 or M1 document. 
Per person, we will list our contributions to this project chronologically.

# Manraj
- Organized each weekly meeting, including the group contract
- Added initial folders in repo to meet M0 requirements
- Added project tech stack section of M0 proposal
- Added AI declaration form
- Submitted M0
- Created M1 doc on google drive, outline for submission

# Diego
- Wrote group contract
- Developed majority of Planit proposal:
    - Project summary, problems being solved, importance
    - 4 potential APIs and 3 features for each
- Added AI declaration form

- Completed the Paid API agreement

# Jiwei
- Developed User flow diagram for M0
- Developed Low fidelity wireframe for M0
- Added AI declaration form
- Created filesystem for repo
- Added interactive wireframe through react.js

# Nahum
- Added user stories, personas for M0
- Added AI declaration form
- Made risk assessment for M1
- Created appendix, again for M1

# Changelog
- tbd